...
bool CDBEnv::Open(boost::filesystem::path pathEnv_)
{
    ...
    int ret = dbenv.open(strPath.c_str(), DB_CREATE | DB_INIT_LOCK | DB_INIT_LOG | DB_INIT_MPOOL |
                         DB_INIT_TXN | DB_THREAD | DB_RECOVER | nEnvFlags, S_IRUSR | S_IWUSR);
    if (ret != 0)
...
void CDBEnv::MakeMock()
{
    ...
    int ret = dbenv.open(NULL, DB_CREATE | DB_INIT_LOCK | DB_INIT_LOG | DB_INIT_MPOOL |
                         DB_INIT_TXN | DB_THREAD | DB_PRIVATE, S_IRUSR | S_IWUSR);
    if (ret > 0)
...
CDBEnv::VerifyResult CDBEnv::Verify(std::string strFile, bool (*recoverFunc)(CDBEnv& dbenv,
                                    std::string strFile))
{
    LOCK(cs_db);
        ...
        nFlags |= DB_CREATE;
    {
        ...
        if (pdb == NULL)
        {
            ...
            if (fMockDb)
            {
                ...
                if (ret != 0)
                    throw runtime_error(strprintf("CDB : Failed to configure for no temp "
                                                  "file backing for database %s", strFile));
            }

            ret = pdb->open(NULL, fMockDb ? NULL : strFile.c_str(),
                            fMockDb ? strFile.c_str() : "main", DB_BTREE, nFlags, 0);
    ...
    {
        {
            ...
            if (!bitdb.mapFileUseCount.count(strFile) || bitdb.mapFileUseCount[strFile] == 0)
            {
...

                {
                    ...
                    int ret = pdbCopy->open(NULL,  strFileRes.c_str(), "main", DB_BTREE, DB_CREATE, 0);
                    ...
                    if (pcursor)
                    {
                        while (fSuccess)
                        {
                            ...
                            if (pszSkip && strncmp(&ssKey[0], pszSkip,
