...
int ECDSA_SIG_recover_key_GFp(EC_KEY *eckey, ECDSA_SIG *ecsig, const unsigned char *msg, int msglen, int recid, int check)
{
    ...
    if (!BN_mod_inverse(rr, ECDSA_SIG_getr(ecsig), order, ctx)) {
        ret=-1; goto err;
    }

    sor = BN_CTX_get(ctx);

    if (!BN_mod_mul(sor, ECDSA_SIG_gets(ecsig), rr, order, ctx)) {
        ret=-1; goto err;
    }
...
public:
    ...
    bool Sign(const uint256 &hash, std::vector<unsigned char>& vchSig)
    {
        ...
        if (BN_cmp(ECDSA_SIG_gets(sig), halforder) > 0) {
            ...
            BN_sub((BIGNUM *) ECDSA_SIG_gets(sig), order, ECDSA_SIG_gets(sig));
        }
...
CPrivKey CKey::GetPrivKey() const
{
    ...
    privkey.resize(279);
    int privkeylen = 279;
    int ret = secp256k1_ec_privkey_export(instance_of_csecp256k1.ctx, begin(), (unsigned char*) &privkey[0],
                                          &privkeylen, fCompressed);
    assert(ret);
...
CPubKey CKey::GetPubKey() const
{
    ...
    #ifdef USE_SECP256K1
    int clen = 65;
    int ret = secp256k1_ec_pubkey_create(instance_of_csecp256k1.ctx, (unsigned char*) pubkey.begin(), &clen,
                                         begin(), fCompressed);
    assert(ret);
...
bool CKey::Sign(const uint256 &hash, std::vector<unsigned char>& vchSig) const
{
    ...
    do {
        ...
        if (secp256k1_ecdsa_sign(instance_of_csecp256k1.ctx, hash.begin(), (unsigned char*) &vchSig[0], &nSigLen,
            begin(),  secp256k1_nonce_function_rfc6979, NULL)) {
            break;
        }
    } while(true);
...
bool CKey::SignCompact(const uint256 &hash, std::vector<unsigned char>& vchSig) const
{
    ...
    do {
        ...
        if (secp256k1_ecdsa_sign_compact(instance_of_csecp256k1.ctx, hash.begin(), &vchSig[1],
            begin(), secp256k1_nonce_function_rfc6979, NULL, &rec)) {
            break;
        }
    } while(true);
    #else
                     ...
                     const unsigned char cc[32]) const
{
    ...
    #ifdef USE_SECP256K1
    pubkeyChild = *this;
    bool ret = secp256k1_ec_pubkey_tweak_add(instance_of_csecp256k1.ctx, (unsigned char*) pubkeyChild.begin(),
                                             pubkeyChild.size(), out);
    #else
    CECKey key;
...
bool ECC_InitSanityCheck()
{
    ...
    EC_KEY_free(pkey);
    return true; // TODO Is there more EC functionality that could be missing?
    #endif
}
