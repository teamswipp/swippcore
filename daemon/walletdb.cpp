...
bool CWalletDB::WriteKey(const CPubKey& vchPubKey, const CPrivKey& vchPrivKey, const CKeyMetadata& keyMeta)
{
    ...
    return Write(std::make_pair(std::string("key"), vchPubKey), std::make_pair(vchPrivKey,
                 Hash(vchKey.begin(), vchKey.end())), false);
}
...
DBErrors CWalletDB::ReorderTransactions(CWallet* pwallet)
{
    ...
    // Get all CWalletTx and CAccountingEntry into a sorted-by-time multimap.
                  ...
                  CWalletScanState &wss, string& strType, string& strErr)
{
    try
    {
        ...
        else if (strType == "tx")
        {
            ...
            if(fDebug)
            {
                LogPrintf("LoadWallet %s\n%12d %s %s\n", wtx.GetHash().ToString(), wtx.vout[0].nValue,
                          wtx.hashBlock.ToString(), wtx.mapValue["message"]);
            }
        } else
        if (strType == "sxAddr")
        {
        ...
        else if (strType == "pool")
        {
            ...
            // If no metadata exists yet, create a default with the pool key's creation time. Note that
            // this may be overwritten by actually stored metadata for that key later, which is fine.
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
        ...
        while (true)
        {
            ...
            if (!ReadKeyValue(pwallet, ssKey, ssValue, wss, strType, strErr))
            {
                ...
                else
                {
                    ...
                    if (strType == "tx")
                        SoftSetBoolArg("-rescan", true); // Rescan if there is a bad transaction record
                }
            }
    ...
    // Any wallet corruption at all: skip any rewriting or upgrading, we don't want to make it worse
    if (result != DB_LOAD_OK)
        return result;

    LogPrintf("nFileVersion = %d\nKeys: %u plaintext, %u encrypted, %u w/ metadata, %u total\n",
              wss.nFileVersion, wss.nKeys, wss.nCKeys, wss.nKeyMeta, wss.nKeys + wss.nCKeys);
