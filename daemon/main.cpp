...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree, bool* pfMissingInputs)
{
    ...
    }
    {
        ...
        if (nSigOps > MAX_TX_SIGOPS)
            return tx.DoS(0, error("AcceptToMemoryPool : too many sigops %s, %d > %d",
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
    ...
    if (hash > bnTarget.getuint256())
    {
        ...
        if (!TestNet() || (TestNet() && pindexBest != NULL))
            return error("CheckProofOfWork() : hash doesn't match nBits (%s vs %s)",
                         hash.ToString(), bnTarget.getuint256().ToString());
    }
    ...
    BOOST_FOREACH(CNode* pn, vNodes)
    {
        if (pn->GetId() == pnode)
        {
            ...
            if (pn->nMisbehavior >= banscore && pn->nMisbehavior - howmuch < banscore)
            {
                ...
                //pn->fShouldBan = true; TODO: Enable ?
            }
            else
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    uint256 nBestBlockTrust = pindexBest->nHeight != 0 ?
                              pindexBest->nChainTrust - pindexBest->pprev->nChainTrust : pindexBest->nChainTrust;

    LogPrintf("InvalidChainFound: invalid block=%s  height=%d  trust=%s  blocktrust=%d  date=%s\n",
              pindexNew->GetBlockHash().ToString(), pindexNew->nHeight, CBigNum(pindexNew->nChainTrust).ToString(),
              nBestInvalidBlockTrust.Get64(), DateTimeStrFormat("%x %H:%M:%S", pindexNew->GetBlockTime()));

    LogPrintf("InvalidChainFound:  current best=%s  height=%d  trust=%s  blocktrust=%d  date=%s\n",
              hashBestChain.ToString(), nBestHeight, CBigNum(pindexBest->nChainTrust).ToString(),
              nBestBlockTrust.Get64(), DateTimeStrFormat("%x %H:%M:%S", pindexBest->GetBlockTime()));
}
...
static bool BuildAddrIndex(const CScript &script, boost::container::flat_set<std::tuple<uint160, uint256>>& addrIds, uint256& hashTx)
{
    ...
    while (pc < pend)
    {
        ...
        if (0 <= opcode && opcode <= OP_PUSHDATA4 && data.size() >= 8)
        {
            ...
            addrIds.insert(std::make_tuple(addrid, hashTx));
            fHaveData = true;
        }
    }
...
bool static Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    LogPrintf("REORGANIZE: Connect %u blocks; %s..%s\n", vConnect.size(), pfork->GetBlockHash().ToString(),
...
bool CBlock::AcceptBlock(bool bootstrap)
{
    AssertLockHeld(cs_main);

    if (!bootstrap && nVersion > CURRENT_VERSION)
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!fReindex && !fImporting && pblock->IsProofOfStake() &&
        setStakeSeen.count(pblock->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash))
    {
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        LogPrintf("%d (%u,%u) %s  %08x  %s  mint %7s  tx %u", pindex->nHeight, pindex->nFile, pindex->nBlockPos,
                  block.GetHash().ToString(), block.nBits, DateTimeStrFormat("%x %H:%M:%S", block.GetBlockTime()),
                  FormatMoney(pindex->nMint), block.vtx.size());
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end())
    {
        ...
        catch (std::ios_base::failure& e)
        {
            if (strstr(e.what(), "end of data"))
            {
                ...
                LogPrintf("ProcessMessages(%s, %u bytes) : Exception '%s' caught, normally caused by a message being "
                          "shorter than its stated length\n", strCommand, nMessageSize, e.what());
            }
