...
Value smsgoptions(const Array& params, bool fHelp)
{
    ...
    else if (mode == "set")
    {
        ...
        if (optname == "newAddressRecv")
        {
            ...
            result.push_back(Pair("set option", std::string("newAddressRecv = ") +
                                  (smsgOptions.fNewAddressRecv ? "true" : "false")));
        }
        else if (optname == "newAddressAnon")
        {
            ...
            result.push_back(Pair("set option", std::string("newAddressAnon = ") +
                                  (smsgOptions.fNewAddressAnon ? "true" : "false")));
        }
        else
        {
...

    {
        ...
        else if (mode == "all" || mode == "unread")
        {
            ...
            while (dbInbox.NextSmesg(it, sPrefix, chKey, smsgStored))
            {
                ...
                if (SecureMsgDecrypt(false, smsgStored.sAddrTo, &smsgStored.vchMessage[0],
                                     &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
                {
                    Object objM;
...

    {
        ...
        else if (mode == "all")
        {
            ...
            while (dbOutbox.NextSmesg(it, sPrefix, chKey, smsgStored))
            {
                ...
                if (SecureMsgDecrypt(false, smsgStored.sAddrOutbox, &smsgStored.vchMessage[0],
                                     &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
                {
                    Object objM;
