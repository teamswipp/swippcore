...
// When performing a read, if we have an active batch we need to check it first before reading from
// the database, as the rest of the code assumes that once a database transaction begins reads are
// consistent with it. It would be good to change that assumption in future and avoid the performance hit,
// though in practice it does not appear to be large
            ...
            LogPrintf("SecureMsgThread %d \n", now);
        {
            ...
            for (std::map<int64_t, SecMsgBucket>::iterator it(smsgBuckets.begin()); it != smsgBuckets.end(); it++)
            {
                ...
                if (it->first < cutoffTime)
                {
                    ...
                    // wl file stores incoming messages when wallet is locked
                ...
                else if (it->second.nLockCount > 0) // Expire if peer never sends data
                {
...
void ThreadSecureMsgPow()
{
    ...
    while (fSecMsgEnabled)
    {
        ...
        for (;;)
        {
            ...
            SecureMsgScanMessage(pHeader, pPayload, psmsg->nPayload, true);
        }
        {
            LOCK(cs_smsg);
            delete it;
        ...
        MilliSleep(2000); // Shutdown thread waits 5 seconds, this should be less
    }
}
...
int SecureMsgBuildBucketSet()
{
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        // TODO: Files must be split if > 2GB (time_noFile.dat)
        ...
        LogPrintf("SecureMsgReceiveData() %s %s.\n", pfrom->addrName.c_str(), strCommand.c_str());
    {
        if (strCommand == "smsgInv")
        {
            ...
            for (uint32_t i = 0; i < nInvBuckets; ++i)
            {
                ...
                if (fDebugSmsg)
                {
                    ...
                    LogPrintf("this bucket %d %u %u.\n", time, smsgBuckets[time].setTokens.size(),
                              smsgBuckets[time].hash);
                }
                {
                    LOCK(cs_smsg);
        ...
        else if (strCommand == "smsgHave")
        {
            ...
            if (vchDataOut.size() > 8)
            {
                ...
                {
                    LOCK(cs_smsg);
                    smsgBuckets[time].nLockCount   = 3; // Lock this bucket for at most 3 * SMSG_THREAD_DELAY seconds
            ...
            std::map<int64_t, SecMsgBucket>::iterator itb;
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    else
                    {
                        ...
                        if (fDebugSmsg)
                            LogPrintf("Have message at %d.\nwinb before SecureMsgRetrieve %d.\n",
                                      it->offset, token.timestamp);
...
bool SecureMsgSendData(CNode* pto, bool fSendTrickle)
{
    ...
    else if (!pto->smsgData.fEnabled || now - pto->smsgData.lastSeen < SMSG_SEND_DELAY ||
...
// Returns (0, success), (1, error), (4, address is already in db)
...
static bool ScanBlock(CBlock& block, CTxDB& txdb, SecMsgDB& addrpkdb, uint32_t& nTransactions,
                      uint32_t& nElements, uint32_t& nPubkeys, uint32_t& nDuplicates)
{
                ...
                while (pc < pend)
                {
                    ...
                    if (fDebugSmsg)
                        LogPrintf("opcode %d, %s, value %s.\n", opcode,
                                  GetOpName(opcode), ValueString(vch).c_str());
                }
...
int SecureMsgWalletUnlocked()
{
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        // TODO: Files must be split if > 2GB (time_noFile_wl.dat)
        size_t sep = fileName.find_first_of("_");
...
// Check if message belongs to this node. If so add to inbox db
// Returns (0, success), (1, error), (2, no match), (3, wallet is locked)
int SecureMsgScanMessage(uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, bool reportToGui)
{
    ...
    for (std::vector<SecMsgAddress>::iterator it = smsgAddresses.begin(); it != smsgAddresses.end(); ++it)
    {
        ...
        else if (SecureMsgDecrypt(true, addressTo, pHeader, pPayload, nPayload, msg) == 0)
        {
            if (fDebugSmsg)
                LogPrintf("Decrypted message with %s.\n", addressTo.c_str());
...
// Returns (0, success), (1, error), (2, invalid address), (3, address does not refer to a key),
...
// Add address and matching public key to the database address and publicKey are in base58
// Returns (0, success), (1, error), (2, publicKey is invalid), (3, publicKey != address),
// (4, address is already in db), (5, address is invalid)
int SecureMsgAddAddress(std::string& address, std::string& publicKey)
{
    ...
    if (!coinAddress.IsValid())
    {
        LogPrintf("SecureMsgAddAddress() - Address is not valid: %s.\n", address.c_str());
        return 5;
    }

    if (!coinAddress.GetKeyID(hashKey))
    {
        LogPrintf("SecureMsgAddAddress() - coinAddress.GetKeyID failed: %s.\n",
                  coinAddress.ToString().c_str());
        return 5;
    }
    ...
    {
        LogPrintf("SecureMsgAddAddress() - Public key does not hash to address, addressT %s.\n", 
...
int SecureMsgReceive(CNode* pfrom, std::vector<uint8_t>& vchData)
{
    ...
    for (uint32_t i = 0; i < nBunch; ++i)
    {
        ...
        }
        {
            ...
            SecureMsgScanMessage(&vchData[n], &vchData[n + SMSG_HDR_LEN], psmsg->nPayload, true);
        }
...
// Returns (0, success), (1, error), (2, invalid hash), (3, checksum mismatch),
// (4, invalid version), (5, payload is too large)
...
// Proof of work and checksum. May run in a thread, if shutdown detected, return
// Returns (0, success), (1, error), (2, stopped due to node shutdown)
...
// Returns (2, message is too long), (3, addressFrom is invalid), (4, addressTo is invalid),
// (5, could not get public key for addressTo), (6, ECDH_compute_key failed),
// (7, could not get private key for addressFrom), (8, could not allocate memory),
...
int SecureMsgEncrypt(SecureMessage& smsg, std::string& addressFrom, std::string& addressTo, std::string& message)
{
    ...
    if (SecureMsgGetStoredKey(ckidDest, cpkDestK) != 0 && SecureMsgGetLocalKey(ckidDest, cpkDestK) != 0)
    {
    ...
    else
    {
        ...
        // Compact signature proves ownership of from address and allows the public key to be recovered
...
int SecureMsgSend(std::string& addressFrom, std::string& addressTo, std::string& message, std::string& sError)
{
    ...
    else
    {
        ...
        else
        {
            ...
            memcpy(&chKey[10], &smsgForOutbox.pPayload, 8);
...
// Returns (1, error), (2, unknown version number), (3, decrypt address is not valid),
...
int SecureMsgDecrypt(bool fTestOnly, std::string& address, uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, MessageData& msg)
{
    ...
    if (lenPlain > 128)
    {
        if (LZ4_decompress_safe((char*) pMsgData, (char*) &msg.vchMessage[0],
