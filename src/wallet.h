...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 1000 Swipp at the same time
};
...
private:
    ...
    int nWalletVersion; // Current wallet version: clients below this version are not able to load the wallet
    int nWalletMaxVersion; // Maximum wallet format version: specifies to what version this wallet may be upgraded
...
public:
    ...
    bool SelectCoinsDark(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet,
                         int nDarksendRoundsMin, int nDarksendRoundsMax) const;

    bool SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet,
                                    vector<COutput>& vCoins, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax);
    ...
    bool SelectCoinsMinConfByCoinAge(int64_t nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs,
                                     std::vector<COutput> vCoins, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet,
                                     int64_t& nValueRet) const;
    ...
    int ScanForWalletTransactions(CBlockIndex* pindexStart, int height = -1, bool fUpdate = false);
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey,
                           int64_t& nFeeRet, int32_t& nChangePos, std::string& strFailReason, const CCoinControl *coinControl=NULL,
                           AvailableCoinsType coin_type=ALL_COINS, bool useIX=false);

    bool CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey,
...
public:
    ...
    void GetAmounts(std::list<boost::tuple<CTxDestination, int64_t, int> >& listReceived,
                    std::list<boost::tuple<CTxDestination, int64_t, int> >& listSent, int64_t& nFee, std::string& strSentAccount) const;
