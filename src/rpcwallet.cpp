...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
    {
        throw runtime_error(
            ...
            "Returns the current Swipp address for receiving payments to this account.");
    }
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error(
            "setaccount <Swippaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error(
            "sendtoaddress <Swippaddress> <amount> [comment] [comment-to] [narration]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error(
            "signmessage <Swippaddress> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error(
            "getreceivedbyaddress <Swippaddress> [minconf=1]\n"
            "Returns the total amount received by <Swippaddress> in transactions with at least [minconf] confirmations.");
    }
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error(
            "sendfrom <fromaccount> <toSwippaddress> <amount> [minconf=1] [comment] [comment-to] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
    }
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Swipp address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        throw runtime_error(
            ...
            "each key is a Swipp address or hex-encoded public key\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    return "wallet encrypted; Swipp server stopping, restart to run with encrypted wallet. "
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
    {
        throw runtime_error(
            ...
            "Returns a new Swipp stealth address for receiving payments anonymously.  ");
    }
...
Value importstealthaddress(const Array& params, bool fHelp)
{
    ...
    else if (!DecodeBase58(sScanSecret, vchScanSecret))
    ...
    else if (!DecodeBase58(sSpendSecret, vchSpendSecret))
