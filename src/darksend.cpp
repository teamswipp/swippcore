...
CDarkSendSigner darkSendSigner; // A helper object for signing messages from masternodes
CActiveMasternode activeMasternode;

std::vector<CDarksendQueue> vecDarksendQueue; // Current darksends in progress on the network
std::vector<CTxIn> vecMasternodesUsed;
map<uint256, CDarksendBroadcastTx> mapDarksendBroadcastTxes; // K eep track of any scanning errors
...
void ProcessMessageDarksend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "dsa") //DarkSend Acceptable
    {
        ...
        if (darkSendPool.sessionUsers == 0)
        {
            if (vecMasternodes[mn].nLastDsq != 0 && vecMasternodes[mn].nLastDsq +
               CountMasternodesAboveProtocol(getBlockVersion(PEER, pindexBest->nHeight)) / 5 > darkSendPool.nDsqCount)
            {
    ...
    else if (strCommand == "dsq") // DarkSend Queue
    {
        ...
        if (!dsq.GetAddress(addr) || !dsq.CheckSignature() || dsq.IsExpired())
            return;
    ...
    {
        if (!isVersionCompatible(PEER, pfrom->nVersion, pindexBest->nHeight) || !fMasterNode)
            return;
...
int GetInputDarksendRounds(CTxIn in, int rounds)
{
    ...
    {
        // Bounds check
        if (in.prevout.n >= tx.vout.size())
            return -4;

        if (tx.vout[in.prevout.n].nValue == DARKSEND_FEE)
            return -3;

        // Make sure the final output is non-denominate
        if (rounds == 0 && !pwalletMain->IsDenominatedAmount(tx.vout[in.prevout.n].nValue))
            return -2;
...
{
    if (fDebug)
        LogPrintf("CDarkSendPool::Check() - entries count %lu\n", entries.size());

    // If entries is full, then move on to the next phase
    if (state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions())
    {
        if (fDebug)
            LogPrintf("CDarkSendPool::Check() -- ACCEPTING OUTPUTS\n");
    ...
    {
        if (fDebug)
            LogPrintf("CDarkSendPool::Check() -- FINALIZE TRANSACTIONS\n");
    ...
    {
        if (fDebug)
            LogPrintf("CDarkSendPool::Check() -- SIGNING\n");
    ...
    if (fMasterNode)
    {
        ...
        if (state == POOL_STATUS_ACCEPTING_ENTRIES)
        {
            BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral)
            {
                ...
                if (!found)
                {
                    LogPrintf("CDarkSendPool::ChargeFees -- found uncooperative node "
                              "(didn't send transaction). Found offence.\n");
                    offences++;
                }
            }
        }
        ...
        // Mostly offending?
        if (offences >= POOL_MAX_TRANSACTIONS-1 && r > 33)
            return;

        // Everyone is an offender? That's not right
        if (offences >= POOL_MAX_TRANSACTIONS)
            return;
        ...
        if (state == POOL_STATUS_ACCEPTING_ENTRIES)
        {
            BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral)
            {
                ...
                {
                    LogPrintf("CDarkSendPool::ChargeFees -- found uncooperative node "
                              "(didn't send transaction). Charging fees.\n");
...
void CDarkSendPool::ChargeRandomFees()
{
    if (fMasterNode)
    {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral)
        {
            ...
            //  1 in 50 successful transactions are charged.
            if (r <= 20)
            {
...
void CDarkSendPool::CheckTimeout()
{
    ...
    if (!fMasterNode)
    {
        ...
        {
            if (fDebug)
                LogPrintf("CDarkSendPool::CheckTimeout() -- Session complete -- Running Check()\n");
    ...
    // If we're the client, give the server a few extra seconds before resetting
    if (!fMasterNode)
        addLagTime = 5000;
    ...
    {
        if (fDebug)
            LogPrintf("CDarkSendPool::CheckTimeout() -- Session timed out (30s) -- resetting\n");
    ...
    {
        if (fDebug)
            LogPrintf("CDarkSendPool::CheckTimeout() -- Session timed out -- restting\n");
...
bool CDarkSendPool::SignatureValid(const CScript& newSig, const CTxIn& newVin)
{
    ...
    if (found >= 0)
    {
        ...
        if (fDebug)
            LogPrintf("CDarkSendPool::SignatureValid() - Sign with sig %s\n",
                      newSig.ToString().substr(0,24).c_str());

        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, SCRIPT_VERIFY_P2SH |
            SCRIPT_VERIFY_STRICTENC, SignatureChecker(txNew, i)))
        {
            if (fDebug)
                LogPrintf("CDarkSendPool::SignatureValid() - Signing - Error signing input %u\n", n);
    ...
    if (fDebug)
        LogPrintf("CDarkSendPool::SignatureValid() - Signing - Succesfully signed input\n");
...
{
    if (txCollateral.vout.size() < 1 || txCollateral.nLockTime != 0)
        return false;
    ...
    {
        if (fDebug)
            LogPrintf ("CDarkSendPool::IsCollateralValid - Unknown inputs in collateral transaction - %s\n",
                       txCollateral.ToString().c_str());
        return false;
    }
    ...
    {
        if (fDebug)
            LogPrintf ("CDarkSendPool::IsCollateralValid - did not include enough fees in transaction %d\n%s\n",
...
bool CDarkSendPool::AddEntry(const std::vector<CTxIn>& newInput, const int64_t& nAmount,
                             ...
                             const std::vector<CTxOut>& newOutput, std::string& error)
{
    ...
    BOOST_FOREACH(CTxIn in, newInput)
    {
        ...
        {
            if (fDebug)
                LogPrintf ("CDarkSendPool::AddEntry - input not valid!\n");
    ...
    {
        if (fDebug)
            LogPrintf ("CDarkSendPool::AddEntry - collateral not valid!\n");
    ...
    {
        if (fDebug)
            LogPrintf ("CDarkSendPool::AddEntry - entries is full!\n");
    ...
    BOOST_FOREACH(CTxIn in, newInput)
    {
        ...
        BOOST_FOREACH(const CDarkSendEntry v, entries)
        {
            BOOST_FOREACH(const CDarkSendEntryVin s, v.sev)
            {
                ...
                {
                    if (fDebug)
                        LogPrintf ("CDarkSendPool::AddEntry - found in vin\n");
    ...
    if (fDebug)
        LogPrintf ("CDarkSendPool::AddEntry - can't accept new entry, wrong state!\n");
...
bool CDarkSendPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted,
                                 std::string& error, int newSessionID)
{
    if (fMasterNode || state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS)
        return false;
...
{
    if (fDebug)
        LogPrintf("CDarkSendPool::AddFinalTransaction - Got Finalized Transaction\n");
    ...
    BOOST_FOREACH(const CDarkSendEntry e, myEntries)
    {
        BOOST_FOREACH(const CDarkSendEntryVin s, e.sev)
        {
            ...
            if (mine >= 0)
            {
                ...
                {
                    LogPrintf("CDarkSendPool::Sign - My entries are not correct! "
                              "Refusing to sign. %d entries %d target. \n", foundOutputs, targetOuputs);

                    return false;
                }

                if (fDebug)
                    LogPrintf("CDarkSendPool::Sign - Signing my input %i\n", mine);

                if (!SignSignature(*pwalletMain, prevPubKey, finalTransaction, mine,
                                   int(SIGHASH_ALL | SIGHASH_ANYONECANPAY)))
                {
                    if (fDebug)
                        LogPrintf("CDarkSendPool::Sign - Unable to sign my own transaction! \n");

                    // TODO: Not sure what to do here. It will perhaps timeout?
                }
                ...
                if (fDebug)
                    LogPrintf(" -- dss %d %d %s\n", mine, (int)sigs.size(),
                              finalTransaction.vin[mine].scriptSig.ToString().c_str());
            }
        }

        if (fDebug)
            LogPrintf("CDarkSendPool::Sign - txNew:\n%s", finalTransaction.ToString().c_str());
    }
...
{
    if (fDebug)
        LogPrintf("CDarkSendPool::NewBlock \n");
...
// TODO: Why only for QT ?

bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    LOCK(cs_darksend);

    if (IsInitialBlockDownload() || fMasterNode || state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS)
        return false;
    ...
    {
        if (fDebug)
            LogPrintf("CDarkSendPool::DoAutomaticDenominating - No masternodes detected\n");
    ...
    if (nBalanceNeedsAnonymized > nBalanceNotYetAnonymized)
        nBalanceNeedsAnonymized = nBalanceNotYetAnonymized;
    ...
    if (fDebug)
        LogPrintf("DoAutomaticDenominating : nLowestDenom=%d, nBalanceNeedsAnonymized=%d\n",
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nDarksendRounds))
    {
        ...
        {
            if (!fDryRun)
                return CreateDenominated(nBalanceNeedsAnonymized);
    ...
    if (!sessionFoundMasternode)
    {
        ...
        if (sessionTotalValue > nBalanceNeedsAnonymized)
            sessionTotalValue = nBalanceNeedsAnonymized;

        double fSwippSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Swipp CENT - sessionTotalValue %d\n", fSwippSubmitted, sessionTotalValue);
        ...
        if (nUseQueue > 33)
        {
            ...
            BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue)
            {
                ...
                if (dsq.time == 0 || !dsq.GetAddress(addr) || dsq.IsExpired() || !dsq.GetProtocolVersion(protocolVersion) ||
                    !isVersionCompatible(INSTANTX, protocolVersion, pindexBest->nHeight))
                    continue;
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized,
                                                             vCoins, vCoins2, nValueIn, 0, nDarksendRounds))
                {
                ...
                if (ConnectNode((CAddress) addr, NULL, true))
                {
                    ...
                    {
                        if ((CNetAddr) pnode->addr != (CNetAddr) submittedToMasternode)
                            continue;
        ...
        while (i < 10)
        {
            ...
            if (vecMasternodes[i].nLastDsq != 0 && vecMasternodes[i].nLastDsq +
                CountMasternodesAboveProtocol(getBlockVersion(PEER, pindexBest->nHeight)) / 5 > darkSendPool.nDsqCount)
            {
                i++;
                continue;
            ...
            if (ConnectNode((CAddress) vecMasternodes[i].addr, NULL, true))
            {
                ...
                BOOST_FOREACH(CNode* pnode, vNodes)
                {
                    if ((CNetAddr) pnode->addr != (CNetAddr) vecMasternodes[i].addr)
                        continue;
...
bool CDarkSendPool::SendRandomPaymentToSelf()
{
    ...
    int32_t nChangePos;
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, nChangePos,
                                                  strFail, coinControl, ONLY_DENOMINATED);
    if (!success)
    {
...
bool CDarkSendPool::MakeCollateralAmounts()
{
    ...
    int32_t nChangePos;
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, nChangePos,
                                                  strFail, coinControl, ONLY_NONDENOMINATED_NOTMN);
    if (!success)
    {
...
bool CDarkSendPool::CreateDenominated(int64_t nTotalValue)
{
    ...
    // If we have anything left over, it will be automatically send back as change
    // There is no need to send it manually
...
bool CDarkSendPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral, std::string& strReason)
{
    ...
    {
        if (fDebug)
            LogPrintf ("CDarkSendPool::IsCompatibleWithSession - collateral not valid!\n");
...
inline void GetSubDenominationsToString(int nDenom, std::string& strDenomComplete, std::string strDenom, int mask)
{
    if (nDenom & mask)
    {
        if (strDenomComplete.size() > 0)
            strDenomComplete += "+";

        strDenomComplete += strDenom;
    }
}

// Create a nice string to show the denominations
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenomComplete){

    // bit 0 | 100 Swipp + 1 (on if present)
    // bit 1 |  10 Swipp + 1
    // bit 2 |   1 Swipp + 1
    // bit 3 | 0.1 Swipp + 1
    // bit 3 | non-denom
    ...
    strDenomComplete = "";
    GetSubDenominationsToString(nDenom, strDenomComplete, "100", 1 << 0);
    GetSubDenominationsToString(nDenom, strDenomComplete, "10", 1 << 1);
    GetSubDenominationsToString(nDenom, strDenomComplete, "1", 1 << 2);
    GetSubDenominationsToString(nDenom, strDenomComplete, "0.1", 1 << 3);
}
...
int CDarkSendPool::GetDenominations(const std::vector<CTxOut>& vout)
{
    ...
    // If the denomination is used, shift the bit on,then move to the next
    BOOST_FOREACH (PAIRTYPE(int64_t, int)& s, denomUsed)
        denom |= s.second << c++;
...
int CDarkSendPool::GetDenominationsByAmount(int64_t nAmount, int nDenomTarget)
{
    ...
    BOOST_REVERSE_FOREACH(int64_t v, darkSendDenominations)
    {
        if (nDenomTarget != 0)
        {
            ...
            if ((nDenomTarget & (1 << 0)) && v == ((100000 * COIN) + 100000000))
                fAccepted = true;
            else if ((nDenomTarget & (1 << 1)) && v == ((10000 * COIN) + 10000000))
                fAccepted = true;
            else if ((nDenomTarget & (1 << 2)) && v == ((1000 * COIN) + 1000000))
                fAccepted = true;
            else if ((nDenomTarget & (1 << 3)) && v == ((100 * COIN) + 100000))
                fAccepted = true;
            else if ((nDenomTarget & (1 << 4)) && v == ((10 * COIN) + 10000))
                fAccepted = true;
            else if ((nDenomTarget & (1 << 5)) && v == ((1 * COIN) + 1000))
                fAccepted = true;
            else if ((nDenomTarget & (1 << 6)) && v == ((.1 * COIN) + 100))
                fAccepted = true;
...
void ThreadCheckDarkSendPool()
{
    ...
    while (true)
    {
        ...
        if (c % 60 == 0)
        {
            // Something is modifying the coins view without a mempool lock. It causes
        ...
        if (c % 60 * (nLiquidityProvider + 1) == 0)
        {
            if (nLiquidityProvider != 0)
            {
                ...
                if (nRand == 50 + nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8)
                {
                    ...
                    nAnonymizeSwippAmount = (rand() % nLeftToAnon) + 3;
                }
                else
