...
OverviewPage::OverviewPage(QWidget *parent) : QWidget(parent), ui(new Ui::OverviewPage), clientModel(0), walletModel(0),
                                              currentBalance(-1), currentStake(0), currentUnconfirmedBalance(-1),
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        setBalance(model->getBalance(), model->getStake(), model->getUnconfirmedBalance(),
                   model->getImmatureBalance(), model->getAnonymizedBalance());

        connect(model, SIGNAL(balanceChanged(qint64, qint64, qint64, qint64, qint64)), this,
                SLOT(setBalance(qint64, qint64, qint64, qint64, qint64)));
...
void OverviewPage::updateDarksendProgress()
{
    ...
    int64_t nMaxToAnonymize = nAnonymizeSwippAmount*COIN;

    // If it's more than the wallet amount, limit to that.
    if(nMaxToAnonymize > nBalance)
    ...
    convert << "Progress: " << progress << "%, inputs have an average of " << pwalletMain->GetAverageAnonymizedRounds()
            << " of " << nDarksendRounds << " rounds";
...
void OverviewPage::darkSendStatus()
{
    ...
    if(fEnableDarksend && nBestHeight != darkSendPool.cachedNumBlocks)
    {
        ...
        strSettings.prepend(BitcoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(),
                            nAnonymizeSwippAmount * COIN));
    ...
    if(state == POOL_STATUS_ACCEPTING_ENTRIES)
    {
        ...
        else
        {
            ...
            {
                convert << tr("Submitted following entries to masternode:").toStdString()
                        << " " << entries << "/" << darkSendPool.GetMaxPoolTransactions();
            }
            else if(showingDarkSendMessage % 70 <= 50)
            {
                convert << tr("Submitted to masternode, Waiting for more entries").toStdString()
                        << " (" << entries << "/" << darkSendPool.GetMaxPoolTransactions() << " ) .";
            }
    ...
    {
        if(showingDarkSendMessage % 70 <= 10)
            convert << tr("Found enough users, signing ...").toStdString();
...
void OverviewPage::toggleDarksend()
{
    if(!fEnableDarksend)
    {
        ...
        {
            QString strMinAmount(BitcoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), minAmount));
            QMessageBox::warning(this, tr("Darksend"), tr("Darksend requires at least %1 to use.").arg(strMinAmount),
