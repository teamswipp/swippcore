...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSwippAmount"))
        settings.setValue("nAnonymizeSwippAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSwippAmount = settings.value("nAnonymizeSwippAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSwippAmount"))
        SoftSetArg("-anonymizeSwippamount", settings.value("nAnonymizeSwippAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSwippAmount:
            nAnonymizeSwippAmount = value.toInt();
            settings.setValue("nAnonymizeSwippAmount", nAnonymizeSwippAmount);
            emit anonymizeSwippAmountChanged(nAnonymizeSwippAmount);
            break;
        default:
            break;
        }
