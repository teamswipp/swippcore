                                                   ...
                                                   ssl::context& context, const bool fUseSSL)
{
    ...
    acceptor->async_accept(
        ...
        conn->peer,
        boost::bind(&RPCAcceptHandler<BASIC_SOCKET_ACCEPTOR_TEMPLATE>, acceptor,
                    boost::ref(context), fUseSSL, conn, boost::asio::placeholders::error)
    );
}
...
void StartRPCThreads()
{
    ...
    {
        strerr = strprintf(_("An error occurred while setting up the RPC port %u "
                             "for listening on IPv6, falling back to IPv4: %s"), endpoint.port(), e.what());
    }
...
void ServiceConnection(AcceptedConnection *conn)
{
    ...
    while (fRun)
    {
        ...
        if (!HTTPAuthorized(mapHeaders))
        {
            ...
            /* Deter brute-forcing short passwords. If this results in a DoS the user
               really shouldn't have their RPC port exposed. */
