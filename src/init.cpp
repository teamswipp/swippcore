...
std::string HelpMessage()
{
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 35075 or testnet: 15075)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Swipp is probably already running."), strDataDir));
    ...
    if(nAnonymizeSwippAmount > 999999) nAnonymizeSwippAmount = 999999;
    if(nAnonymizeSwippAmount < 2) nAnonymizeSwippAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeSwippAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
