...
bool static ConnectSocketDirectly(const CService &addrConnect, SOCKET& hSocketRet, int nTimeout)
{
    ...
    // We'll turn it back to blocking just in case
#ifdef WIN32
    fNonblock = 0;
...
bool ConnectSocketByName(CService &addr, SOCKET& hSocketRet, const char *pszDest,
                         int portDefault, int nTimeout, bool *outProxyConnectionFailed)
{
    string strDest;
    int port = portDefault;
...
{
    return IsValid() && !(IsRFC1918() || IsRFC3927() || IsRFC4862() || (IsRFC4193() &&
                          !IsTor() && !IsI2P()) || IsRFC4843() || IsLocal());
}
...
std::string CNetAddr::ToStringIP() const
{
    ...
    if (serv.GetSockAddr((struct sockaddr*)&sockaddr, &socklen))
    {
        ...
        if (!getnameinfo((const struct sockaddr*) &sockaddr, socklen, name, sizeof(name),
