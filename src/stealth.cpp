...
uint32_t BitcoinChecksum(uint8_t *p, uint32_t nBytes)
{
    ...
    return from_little_endian<uint32_t>(&hash2[0]);
}
...
bool VerifyChecksum(const data_chunk& data)
{
    ...
    return BitcoinChecksum((uint8_t *) &data[0], data.size() - 4) == from_little_endian<uint32_t>(data.end() - 4);
}
...
int StealthSecret(ec_secret& secret, ec_point& pubkey, const ec_point& pkSpend, ec_secret& sharedSOut, ec_point& pkOut)
{
    ...
    */
    ...
    STEALTH_SECRET_IF(!(bnCtx = BN_CTX_new()), "BN_CTX_new failed.\n");
    STEALTH_SECRET_IF(!(bnEphem = BN_bin2bn(&secret.e[0], ec_secret_size, BN_new())), "bnEphem BN_bin2bn failed.\n");
    STEALTH_SECRET_IF(!(bnQ = BN_bin2bn(&pubkey[0], pubkey.size(), BN_new())), "bnQ BN_bin2bn failed\n");
    STEALTH_SECRET_IF(!(Q = EC_POINT_bn2point(ecgrp, bnQ, NULL, bnCtx)), "Q EC_POINT_bn2point failed\n")
    STEALTH_SECRET_IF(!EC_POINT_mul(ecgrp, Q, NULL, Q, bnEphem, bnCtx), "eQ EC_POINT_mul failed\n");
    STEALTH_SECRET_IF(!(bnOutQ = EC_POINT_point2bn(ecgrp, Q, POINT_CONVERSION_COMPRESSED, BN_new(), bnCtx)),
                      "Q EC_POINT_bn2point failed\n");

    vchOutQ.resize(ec_compressed_size);

    STEALTH_SECRET_IF(BN_num_bytes(bnOutQ) != (int) ec_compressed_size ||
                      BN_bn2bin(bnOutQ, &vchOutQ[0]) != (int) ec_compressed_size, "bnOutQ incorrect length.\n");

    SHA256(&vchOutQ[0], vchOutQ.size(), &sharedSOut.e[0]);

    STEALTH_SECRET_IF(!(bnc = BN_bin2bn(&sharedSOut.e[0], ec_secret_size, BN_new())), "BN_bin2bn failed\n");
    STEALTH_SECRET_IF(!(C = EC_POINT_new(ecgrp)), "C EC_POINT_new failed\n");
    STEALTH_SECRET_IF(!EC_POINT_mul(ecgrp, C, bnc, NULL, NULL, bnCtx), "C EC_POINT_mul failed\n");
    STEALTH_SECRET_IF(!(bnR = BN_bin2bn(&pkSpend[0], pkSpend.size(), BN_new())), "bnR BN_bin2bn failed\n");
    STEALTH_SECRET_IF(!(R = EC_POINT_bn2point(ecgrp, bnR, NULL, bnCtx)), "R EC_POINT_bn2point failed\n");
    STEALTH_SECRET_IF(!EC_POINT_mul(ecgrp, C, bnc, NULL, NULL, bnCtx), "C EC_POINT_mul failed\n");
    STEALTH_SECRET_IF(!(Rout = EC_POINT_new(ecgrp)), "Rout EC_POINT_new failed\n");
    STEALTH_SECRET_IF(!EC_POINT_add(ecgrp, Rout, R, C, bnCtx), "Rout EC_POINT_add failed\n");
    STEALTH_SECRET_IF(!(bnOutR = EC_POINT_point2bn(ecgrp, Rout, POINT_CONVERSION_COMPRESSED, BN_new(), bnCtx)),
                      "Rout EC_POINT_bn2point failed\n");

    pkOut.resize(ec_compressed_size);

    STEALTH_SECRET_IF(BN_num_bytes(bnOutR) != (int) ec_compressed_size ||
                      BN_bn2bin(bnOutR, &pkOut[0]) != (int) ec_compressed_size, "pkOut incorrect length.\n");
End:
    if (bnOutR)
        BN_free(bnOutR);
    if (Rout)
...
int StealthSecretSpend(ec_secret& scanSecret, ec_point& ephemPubkey, ec_secret& spendSecret, ec_secret& secretOut)
{
    ...
    */
    ...
    STEALTH_SECRET_SPEND_IF(!(bnCtx = BN_CTX_new()), "BN_CTX_new failed.\n")
    STEALTH_SECRET_SPEND_IF(!(bnScanSecret = BN_bin2bn(&scanSecret.e[0], ec_secret_size, BN_new())),
                            "bnScanSecret BN_bin2bn failed.\n");

    STEALTH_SECRET_SPEND_IF(!(bnP = BN_bin2bn(&ephemPubkey[0], ephemPubkey.size(), BN_new())), "bnP BN_bin2bn failed\n");
    STEALTH_SECRET_SPEND_IF(!(P = EC_POINT_bn2point(ecgrp, bnP, NULL, bnCtx)), "P EC_POINT_bn2point failed\n");
    STEALTH_SECRET_SPEND_IF(!EC_POINT_mul(ecgrp, P, NULL, P, bnScanSecret, bnCtx), "dP EC_POINT_mul failed\n");
    STEALTH_SECRET_SPEND_IF(!(bnOutP = EC_POINT_point2bn(ecgrp, P, POINT_CONVERSION_COMPRESSED, BN_new(), bnCtx)),
                            "P EC_POINT_bn2point failed\n");

    vchOutP.resize(ec_compressed_size);

    STEALTH_SECRET_SPEND_IF(BN_num_bytes(bnOutP) != (int) ec_compressed_size ||
                            BN_bn2bin(bnOutP, &vchOutP[0]) != (int) ec_compressed_size, "bnOutP incorrect length.\n");

    uint8_t hash1[32];
    SHA256(&vchOutP[0], vchOutP.size(), (uint8_t*)hash1);

    STEALTH_SECRET_SPEND_IF(!(bnc = BN_bin2bn(&hash1[0], 32, BN_new())), "BN_bin2bn failed\n");
    STEALTH_SECRET_SPEND_IF(!(bnOrder = BN_new()) || !EC_GROUP_get_order(ecgrp, bnOrder, bnCtx), "EC_GROUP_get_order failed\n");
    STEALTH_SECRET_SPEND_IF(!(bnSpend = BN_bin2bn(&spendSecret.e[0], ec_secret_size, BN_new())), "bnSpend BN_bin2bn failed.\n");
    STEALTH_SECRET_SPEND_IF(!BN_mod_add(bnSpend, bnSpend, bnc, bnOrder, bnCtx), "bnSpend BN_mod_add failed.\n");
    STEALTH_SECRET_SPEND_IF(BN_is_zero(bnSpend), "bnSpend is zero.\n");
    STEALTH_SECRET_SPEND_IF(BN_num_bytes(bnSpend) != (int) ec_secret_size ||
                            BN_bn2bin(bnSpend, &secretOut.e[0]) != (int) ec_secret_size, "bnSpend incorrect length.\n");
End:
    if (bnSpend)
        BN_free(bnSpend);
    if (bnOrder)
...
int StealthSharedToSecretSpend(ec_secret& sharedS, ec_secret& spendSecret, ec_secret& secretOut)
{
    ...
    STEALTH_SECRET_SPEND_IF(!(bnCtx = BN_CTX_new()), "BN_CTX_new failed.\n");
    STEALTH_SECRET_SPEND_IF(!(bnc = BN_bin2bn(&sharedS.e[0], ec_secret_size, BN_new())), "BN_bin2bn failed\n");
    STEALTH_SECRET_SPEND_IF(!(bnOrder = BN_new()) || !EC_GROUP_get_order(ecgrp, bnOrder, bnCtx), "EC_GROUP_get_order failed\n");
    STEALTH_SECRET_SPEND_IF(!(bnSpend = BN_bin2bn(&spendSecret.e[0], ec_secret_size, BN_new())), "bnSpend BN_bin2bn failed.\n");
    STEALTH_SECRET_SPEND_IF(!BN_mod_add(bnSpend, bnSpend, bnc, bnOrder, bnCtx), "bnSpend BN_mod_add failed.\n");
    STEALTH_SECRET_SPEND_IF(BN_is_zero(bnSpend), "bnSpend is zero.\n");
