...
import "./reacteroids.css";

const KEY = { LEFT: 37, RIGHT: 39, UP: 38, A: 65, D: 68, W: 87, SPACE: 32 };
...
export default class Reacteroids extends Component {
	...
	addScore(points) {
		if (this.state.inGame) {
			this.setState({ currentScore: this.state.currentScore + points });
		}
	}
	...
	generateAsteroids(howMany) {
		...
		for (let i = 0; i < howMany; i++) {
			let asteroid = new Asteroid({
				size: 80,
				position: {
					x: randomNumBetweenExcluding(0, this.state.screen.width,
					                             ship.position.x - 60, ship.position.x + 60),
					y: randomNumBetweenExcluding(0, this.state.screen.height,
					                             ship.position.y - 60, ship.position.y + 60)
				}, create: this.createObject.bind(this),
